...
이 예제는 Crazyflie 여러 대가 P2P 무선으로 서로의 상태를 주고받으며, CBBA에서 영감을 받은 분산 경매 알고리즘으로 작업을 할당하고 실제 비행 경로를 따라가며 task 완료를 퍼뜨리는 데모입니다. 논문의 알고리즘을 그대로 구현한 것이 아니라, 임베디드 P2P 환경에 맞게 단순화/변형한 자체 구현입니다. "통신, 상태 동기화, 분산 할당, 비행 제어"가 한 앱 안에서 어떻게 연결되는지를 보여주는 교육용 코드라고 보면 됩니다.
...
이 예제는 "Crazyflie 여러 대가 P2P로 상태를 공유하면서 CBBA 기반 커스텀 알고리즘으로 task를 나누고, 비행 State Machine 안에서 그 task를 실제로 수행하는 구조"를 보여주는 온보딩용 샘플입니다. 처음에는 통신만 보지 말고, `app_main -> p2p_comm -> cbba_full -> app_main`으로 이어지는 흐름을 따라가면 이해가 빨라집니다.
