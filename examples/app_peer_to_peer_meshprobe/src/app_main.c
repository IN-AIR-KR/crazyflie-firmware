...
static uint8_t boolToU8(bool value) { return value ? 1u : 0u; }
...
static void updatePeerCacheDoneLedger(uint8_t my_radio_low,
                                      const msg_done_ledger_t* ledger) {
  if (ledger->src_id == my_radio_low) {
    return;
  }

  if (my_radio_low == NODE_ID_D1) {
    if (ledger->src_id == NODE_ID_D2) {
      Cbba_UpdatePeerCacheFromDoneLedger(&g_peer1, ledger);
    } else if (ledger->src_id == NODE_ID_D3) {
      Cbba_UpdatePeerCacheFromDoneLedger(&g_peer2, ledger);
    }
  } else if (my_radio_low == NODE_ID_D2) {
    if (ledger->src_id == NODE_ID_D1) {
      Cbba_UpdatePeerCacheFromDoneLedger(&g_peer1, ledger);
    } else if (ledger->src_id == NODE_ID_D3) {
      Cbba_UpdatePeerCacheFromDoneLedger(&g_peer2, ledger);
    }
    ...
    if (ledger->src_id == NODE_ID_D1) {
      Cbba_UpdatePeerCacheFromDoneLedger(&g_peer1, ledger);
    } else if (ledger->src_id == NODE_ID_D2) {
      Cbba_UpdatePeerCacheFromDoneLedger(&g_peer2, ledger);
    }
  }
}

static uint8_t execForRadio(uint8_t radio_low, uint8_t my_radio_low,
...
static void printObserverOneLine(uint8_t my_radio_low, bool aliveD1,
  ...
  const uint8_t active_any =
      ((activeD1 != 0u) || (activeD2 != 0u) || (activeD3 != 0u)) ? 1u : 0u;
  const uint8_t idle_all =
      ((activeD1 == 0u) && (activeD2 == 0u) && (activeD3 == 0u)) ? 1u : 0u;
  const uint8_t stuck =
  ...
  DEBUG_PRINT(
      ...
      (unsigned)active_any, (unsigned)idle_all, (unsigned)stuck,
      (unsigned)global_done_count, (unsigned)TASK_COUNT_RUNTIME,
      (unsigned)om->all_known, (unsigned)om->winner_conv, (unsigned)om->fp_conv,
      (unsigned)om->contested_tasks, (double)worldPos->x_m,
      (double)worldPos->y_m);
}
...
void appMain(void) {
  ...
  DEBUG_PRINT("[APP] START my_id=0x%02X (%s) agent=%u start=(%.2f,%.2f)\n",
              (unsigned)my_radio_low, appNodeName(my_radio_low),
              (unsigned)my_agent_id, (double)startPos.x_m,
              (double)startPos.y_m);
  ...
  while (st != ST_OFF) {
    ...
    if (st == ST_RUN) {
      ...
      while (p2pCommPollEvent(&ev)) {
        ...
        } else if (ev.type == MSG_DONE_LEDGER) {
          Cbba_HandleDoneLedger(&g_cbba, &ev.u.ledger);
          updatePeerCacheDoneLedger(my_radio_low, &ev.u.ledger);
        }
      }
      ...
      if (Cbba_MakeDoneLedgerMsg(&g_cbba, nowMs, &ledgerMsg)) {
        p2pCommSendDoneLedger(&ledgerMsg);
      }
      ...
      if ((TASK_TABLE_DEBUG_ENABLE != 0u) &&
        ...
        Cbba_DebugPrintTables(appNodeName(my_radio_low), &g_cbba, &g_peer1,
                              &g_peer2);
      }
    }
      ...
      if (st == ST_TAKEOFF) {
        ...
        if ((nowT - run_t0) < M2T(POST_TAKEOFF_HOLD_MS)) {
          ...
          const float tgt_local_y =
              g_cbba.tasks[g_cbba.exec_task].pos.y_m - startPos.y_m;
          ...
          if ((nowT - lastGuideLog) >= M2T(300)) {
            ...
            DEBUG_PRINT(
                "[GUIDE] me=%s exec=%u tgt=(%.2f,%.2f) pos=(%.2f,%.2f) "
                "err=(%.2f,%.2f) dist2=%.3f vel=(%.2f,%.2f)\n",
                appNodeName(my_radio_low), (unsigned)g_cbba.exec_task,
                (double)tgt_local_x, (double)tgt_local_y, (double)me.position.x,
                (double)me.position.y, (double)ex, (double)ey,
                (double)dist2_local, (double)vx, (double)vy);
          }
