...
/* ═══════════════════════════════════════════════════════════
 * 패킷 구조체 (CBAA 입찰 + 현재 위치 합산, 30 bytes)
 * ═══════════════════════════════════════════════════════════ */
typedef struct {
...
static logVarId_t log_id_pos_x;
static logVarId_t log_id_pos_y;
static logVarId_t log_id_pos_z;

/* ═══════════════════════════════════════════════════════════
 * cfclient 모니터링용 로그 변수
 * ═══════════════════════════════════════════════════════════ */
static uint8_t log_state = 0;
static uint8_t log_task = 0xFF;
static uint8_t log_iter = 0;
static uint8_t log_stable = 0;
static float log_cx = 0.0f;     /* 현재 x */
static float log_cy = 0.0f;     /* 현재 y */
static float log_apf_tx = 0.0f; /* APF 보정 목표 x */
static float log_apf_ty = 0.0f; /* APF 보정 목표 y */
 ...
 * APF 보정 목표 계산 (xy 평면)
 *
 *   인력(attractive): task 방향으로 당김 → 이미 GoTo 목표로 반영
 *   척력(repulsive) : 이웃 CF로부터 밀어냄
 *
 *   F_rep_j = K_rep * (1/dist - 1/rho0) / dist^2 * (pos - neighbor)
 *             단, dist < rho0 일 때만 적용
 *
 *   최종 GoTo 목표:
 *     tx = task_x + rep_x   (척력 오프셋만 추가; 인력은 GoTo가 담당)
 *     ty = task_y + rep_y
 *
 * ═══════════════════════════════════════════════════════════ */
static void apfCompute(float cx, float cy, float* out_tx, float* out_ty) {
  ...
  for (int k = 0; k < N_AGENTS; k++) {
    ...
    if (dist >= APF_RHO_0) continue; /* 영향 반경 밖 */
...
void appMain(void) {
  ...
  /* 현재 위치 로그 변수 ID 취득 */
  log_id_pos_x = logGetVarId("stateEstimate", "x");
  log_id_pos_y = logGetVarId("stateEstimate", "y");
  log_id_pos_z = logGetVarId("stateEstimate", "z");
  ...
  peer_heard[my_idx] = true; /* 자신은 이미 확인 */
  setAppState(STATE_WAIT_PEERS);
  DEBUG_PRINT("[CBAA] Waiting for all peers...\n");

  for (;;) {
    /* 자신의 초기 상태 브로드캐스트: 다른 드론이 우리를 발견할 수 있도록 */
  ...
  vTaskDelay(M2T(2000)); /* 센서/추정기 안정화 */
  ...
  for (cbaa_iter = 0; cbaa_iter < MAX_CBAA_ITER; cbaa_iter++) {
     ...
     *    전역 좌표 = 상대 좌표 + START_POS (점수·APF·브로드캐스트에 사용)
     *    GoTo 목표는 다시 로컬로 변환 필요 */
    float cx_local = logGetFloat(log_id_pos_x);
    float cy_local = logGetFloat(log_id_pos_y);
    float cz = logGetFloat(log_id_pos_z);
    ...
    /* 6. CBAA_PERIOD_MS 동안 수신 패킷 처리 (10ms 폴링) */
    ...
    /* 8. GoTo 재발행 (매 반복, duration=1.0s → 부드러운 경로 추적)
     *    apf_tx/ty는 전역 좌표 → HL Commander는 로컬 좌표 기준이므로 변환 */
    if (assigned_task >= 0) {
    ...
    /* 10. 수렴 판정: CBAA 안정(N_STABLE_REQUIRED 회 불변) + 목적지 도착
     *     도착 기준: 현재 위치와 목표 위치의 3D 거리 < ARRIVAL_THRESHOLD */
  ...
  /* 최종 GoTo: APF 없이 정확한 목표 위치로 이동 후 도착 대기 */
...
/** 앱 상태: 0=INIT 1=WAIT_PEERS 2=TAKEOFF 3=HOVER_WAIT 4=CBAA_FLY 5=LAND_WAIT 6=LAND 7=DONE */
LOG_ADD(LOG_UINT8, state, &log_state)
/** 현재 배정 태스크 (0xFF=미배정) */
LOG_ADD(LOG_UINT8, task, &log_task)
/** 전체 CBAA 반복 번호 */
LOG_ADD(LOG_UINT8, iter, &log_iter)
/** 연속 안정 반복 수 (5 도달 + 궤적완료 → 수렴) */
